...
let peek p = if p.i < String.length p.text then Some p.text.[p.i] else None

let advance p =
  if p.i < String.length p.text then (
    let c = p.text.[p.i] in
    p.i <- p.i + 1;
    Some c)
  else None
...
let expect_char p c =
  ...
  | Some c' ->
      Error
        (Printf.sprintf "expected '%c' but got '%c' at position %d" c c' p.i)
  ...
  let rec loop () =
    ...
    | Some '\\' -> (
        ...
        | Some 'u' -> (
            ...
            | None ->
                Error
                  (Printf.sprintf "invalid unicode escape \\u%s"
                     (Buffer.contents hex)))
        | Some c ->
...
let rec parse_value p =
  ...
  | Some '[' -> parse_array p
  | Some '"' -> (
      match parse_string p with Ok s -> Ok (String s) | Error e -> Error e)
...
let float_value = function Number f -> Some f | _ -> None
let int_value = function Number f -> Some (int_of_float f) | _ -> None
