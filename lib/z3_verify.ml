...
let i32_to_z3 n = Z3.BitVector.mk_numeral (ctxt ()) (Int32.to_string n) 32
...
let ip4_cidr_low_high cidr =
  ...
  | [ net; plen ] -> (
      ...
      | Some plen when plen >= 0 && plen <= 32 ->
          ...
          let host = 32 - plen in
          let mask =
            if plen = 0 then 0l else Int32.shift_right_logical 0xffffffffl host
          in
                ...
                if String.contains entry '/' then
                  ...
                  | Some (low, high) ->
                      Some
                        (mk_all_of
                           [
                             mk_bv_uge var (i32_to_z3 low);
                             mk_bv_ule var (i32_to_z3 high);
                           ])
                  | None -> None
                else Some (mk_bv_eq var (i32_to_z3 (ip4_to_int32 entry))))
              entries
          in
...
let encode_port (var : Z3.Expr.expr) (range : port_range) =
  ...
  | Range (low, high) ->
      mk_all_of
        [
          Z3.BitVector.mk_uge (ctxt ()) var
            (Z3.BitVector.mk_numeral (ctxt ()) (string_of_int low) 16);
          Z3.BitVector.mk_ule (ctxt ()) var
            (Z3.BitVector.mk_numeral (ctxt ()) (string_of_int high) 16);
        ]
...
let encode_interface (ir : Ir.t) (iface_var : Z3.Expr.expr)
    (rule_iface : interface_ref option) =
  match rule_iface with
  | None -> mk_true ()
  | Some i ->
      let idx = iface_index ir i.device in
      mk_eq iface_var (Z3.Arithmetic.Integer.mk_numeral_i (ctxt ()) idx)
...
let encode_nat_match (ir : Ir.t) (sp : sym_packet) (nat : nat) =
  Z3.Boolean.mk_not (ctxt ())
    sp.pkt_dir (* Out = false in dir encoding, so NOT dir means Out *)
  |> ignore;
  (* NAT fires for outbound traffic on matching interface *)
  let conds =
    [
      mk_eq sp.pkt_dir (Z3.Boolean.mk_true (ctxt ()));
      (* true = out in our encoding *)
...
let encode_rdr_match (ir : Ir.t) (sp : sym_packet) (rdr : rdr) =
  let conds =
    [
      Z3.Boolean.mk_not (ctxt ()) sp.pkt_dir;
      (* false = in *)
...
let match_rule_concrete (ir : Ir.t) (rule : rule) (pkt : Explain.packet) =
  let dir_ok =
    match rule.direction with None -> true | Some d -> d = pkt.direction
  in
  let iface_ok =
  ...
  let proto_ok =
    ...
    | Proto_any -> true
    | Proto_named n -> (
        match pkt.protocol with
        | Proto_any -> false
        | Proto_named pn -> String.equal n pn)
  in
  let src_ok =
            ...
            List.exists
              (fun entry ->
                ...
                | Some (low, high) ->
                    ...
                    Int32.compare pkt_int low >= 0
                    && Int32.compare pkt_int high <= 0
                | None -> false)
              t.entries)
  in
  let dst_ok =
...
let find_matching_rule_line (ir : Ir.t) sp model =
  ...
  let pkt : Explain.packet =
    {
      ...
      interface = model_get_iface ir model sp.pkt_iface;
      protocol =
        (match model_get_proto model sp.pkt_proto with
        | "tcp" -> Proto_named "tcp"
        | "udp" -> Proto_named "udp"
        | "icmp" -> Proto_named "icmp"
        | _ -> Proto_any);
      source = src;
      destination = dst;
      port;
  ...
  let rec find_first = function
    | [] -> None
    | r :: rest ->
        if match_rule_concrete ir r pkt then Some r.span.line
...
let rec encode_clause (ir : Ir.t) (sp : sym_packet) = function
  | Field (field, _op, value) -> (
      ...
      | "dst" | "destination" ->
          mk_bv_eq sp.pkt_dst (i32_to_z3 (ip4_to_int32 value))
      | "dport" | "port" -> (
          match int_of_string_opt value with
          | Some p ->
              mk_eq sp.pkt_port
...
let check_invariant (ir : Ir.t) clauses =
  ...
  let condition = mk_all_of (List.map (encode_clause ir pol.sp) clauses) in
...
let minimize (ir : Ir.t) =
  ...
  List.iteri
    (fun i rule ->
      ...
      let diff =
        mk_or
          (mk_not (mk_eq pass_full pass_minus))
...
let check_backend_equivalence ~ir ~ebpf_rules =
  ...
  let diff =
    mk_or
      (mk_not (mk_eq pass_ir ebpf_pass))
