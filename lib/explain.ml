...
let match_port (range : port_range) (value : int option) =
  ...
  | Port_any -> true
  | Range (low, high) -> (
      match value with Some p -> p >= low && p <= high | None -> false)
...
let find_shadow (ir : Ir.t) (pkt : packet) (matching_rule : rule option) =
  ...
  | Some matched -> (
      ...
      in
      let anchor_rules =
        List.concat_map (fun (a : anchor) -> a.rules) ir.anchors
      in
...
let explain (ir : Ir.t) (pkt : packet) =
  ...
  let matching_rule =
    List.find_opt (fun rule -> match_rule ir rule pkt) ir.rules
  in
...
let explain_to_string e =
  ...
  let lines =
    ...
    | Some r ->
        lines
        @ [
            Printf.sprintf "Matched RDR: line %d (redirect to %s)" r.span.line
              (address_to_string r.translation);
          ]
    | None -> lines
  in
  let lines =
    ...
    | Some n ->
        lines
        @ [
            Printf.sprintf "Matched NAT: line %d (translate to %s)" n.span.line
              (address_to_string n.translation);
          ]
    | None -> lines
  in
  let lines =
    ...
    | Some (gw, iface) ->
        let iface_str =
          match iface with Some i -> " (" ^ i.device ^ ")" | None -> ""
        in
        lines
        @ [ Printf.sprintf "Route-to: %s%s" (address_to_string gw) iface_str ]
    | None -> lines
  in
  let lines =
    match e.queue with
...
let packet_json (pkt : packet) =
  Json_util.field_object
    [
      ( "direction",
        match pkt.direction with
        | In -> Json_util.string "in"
        | Out -> Json_util.string "out" );
      ("interface", Json_util.string pkt.interface);
      ( "protocol",
        match pkt.protocol with
        | Proto_any -> Json_util.string "any"
        | Proto_named n -> Json_util.string n );
  ...
  Json_util.field_object
    [
      ...
      ( "decision",
        ...
        | Pass -> Json_util.string "pass"
        | Block -> Json_util.string "block"
        | Reject -> Json_util.string "reject" );
      ( "matching_rule",
        Json_util.option (Ir_json.rule_json ~include_spans:true) e.matching_rule
      );
