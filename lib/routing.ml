...
let extract_route_targets (ir : Ir.t) =
  let rec extract_rules acc = function
    ...
    | (r : Ir.rule) :: rest ->
        let acc =
          match r.route_to with Some target -> target :: acc | None -> acc
        in
...
let compile (ir : Ir.t) : t =
  ...
  |> List.filter_map (fun (idx, (gateway, iface_opt)) ->
         match gateway with
         | Ir.Literal gateway_str ->
             ...
             let device_str =
               ...
               | Some (i : Ir.interface_ref) -> Some i.device
               | None -> None
             in
             let cmds =
               [
                 ...
                 Ip_route_add_default
                   { gateway = gateway_str; device = device_str; table };
               ]
             in
             Some cmds
         | _ ->
             Printf.eprintf
               "routing: skipping non-literal gateway at target %d\n" idx;
             None)
  |> List.flatten

let string_of_command = function
  | Ip_rule_add { mark; table } ->
      Printf.sprintf "ip rule add fwmark %d table %d" mark table
  ...
  List.iter
    (function
      | Ip_rule_add r -> (
          let found =
            List.find_opt
              (fun (o : Ip.observed_rule) ->
                o.table = r.table
                && match o.fwmark with Some m -> m = r.mark | None -> false)
              observed_rules
          in
          match found with
          | None ->
              ...
              Buffer.add_string buf
                (Printf.sprintf "-rule fwmark %d table %d: missing\n" r.mark
                   r.table);
              Buffer.add_string buf
                (Printf.sprintf "+rule fwmark %d table %d\n" r.mark r.table)
          | Some _ -> ())
      | Ip_route_add_default r -> (
          let found =
            List.find_opt
              (fun (o : Ip.observed_route) ->
          ...
          | None ->
              changes := true;
              Buffer.add_string buf
                (Printf.sprintf "-route default via %s table %d: missing\n"
                   r.gateway r.table);
              Buffer.add_string buf
                (Printf.sprintf "+route default via %s table %d\n" r.gateway
                   r.table)
          | Some _ -> ()))
    intended;
  if not !changes then
    { changes_required = false; text = "routing diff: no changes\n" }
  else
    {
      changes_required = true;
      text = "routing diff: changes required\n" ^ Buffer.contents buf;
    }
