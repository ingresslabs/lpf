...
let shared_files =
  [ "/etc/lpf.conf"; "/var/lib/lpf/history"; "/var/lib/lpf/rollback" ]
  ...
  [
    {
      ...
      description =
        [
          "Parse, type-check, validate, and lower an lpf policy into the typed \
           intermediate representation without changing host state.";
          "Diagnostics must include source locations, shadowed-rule warnings, \
           and actionable recovery guidance.";
        ];
      options =
        [
          ("--json", "emit machine-readable validation status and diagnostics");
        ];
      examples =
        [
    ...
    };
    {
      ...
      description =
        [ "Format policy files deterministically for review and CI." ];
      options =
        [
    ...
    };
    {
      ...
      description =
        [
          ...
          "Backend plans cover nftables table/chain/set/rule generation, \
           policy routing with ip rule/route tables, tc qdisc/class/shaping \
           compilation, and conntrack declarations.";
        ];
      options =
        [
      ...
      files = shared_files;
      safety_notes =
        [ "Planning is read-only but may inspect current host capabilities." ];
    ...
    };
    {
      ...
      section = 8;
      synopsis =
        "lpf diff [--backend nftables|tc|routing] [--observed <path>|--live] \
         [--json] <policy>";
      description =
        [
          "Compare a generated plan with current lpf-owned host state across \
           backends.";
          "nftables backend: reads live state by default, extracts lpf-owned \
           table blocks, and diffs them against rendered intent.";
          "tc backend: reads qdisc/class state per device and performs \
           semantic comparison against compiled plans.";
          "routing backend: reads ip rule and ip route state and performs \
           semantic comparison.";
          "Supplying --observed reads raw state text from a file or stdin for \
           deterministic tests.";
        ];
      options =
        [
          ...
          ( "--backend routing",
            "select policy-routing rule and route-table diffing" );
          ( "--observed <path>",
            "read observed backend text from a file or - for stdin" );
          ( "--live",
            "read observed backend state from the host; this is the default" );
    ...
    };
    {
      ...
      description =
        [
          ...
          "Guarded apply records rollback preimages and automatically rolls \
           back if not confirmed.";
        ];
      options =
        [
          ( "--confirm <duration>",
            "require confirmation before the duration expires" );
          ("--dry-run", "validate and plan without changing host state");
        ];
      examples =
        [
          "lpf apply /etc/lpf.conf --confirm 60s";
          "lpf apply /etc/lpf.conf --dry-run";
        ];
      files = shared_files;
    ...
    };
    {
      ...
      description =
        [
          "Confirm a pending guarded apply and promote it to known-good state.";
        ];
      options = [];
      ...
      safety_notes =
        [
          "Confirm only after verifying management access and expected traffic \
           behavior.";
        ];
    ...
    };
    {
      ...
      description =
        [ "Restore a previous lpf policy state and backend preimages." ];
      options = [ ("--now", "rollback the active guarded apply immediately") ];
      examples = [ "lpf rollback"; "lpf rollback 2026-06-16T20:30:00Z" ];
      files = shared_files;
      safety_notes =
        [
          "Rollback must cover nftables, routes, tc, dynamic tables, and owned \
           sysctls.";
        ];
    ...
    };
    {
      ...
      section = 8;
      synopsis =
        "lpf explain [--json] [in|on <iface>] [from <addr>] [to <addr>] [proto \
         <proto>] [port <port>] <policy>";
      description =
        [
          "Explain how a hypothetical packet would be handled by the policy.";
          "The static evaluator simulates matching across rules, NAT, and \
           redirect sections.";
        ];
      options =
        [
      ...
      examples =
        [
          ...
          "lpf explain in wan from 203.0.113.10 to firewall port 22 \
           /etc/lpf.conf";
          "lpf explain --json from 10.0.0.5 to 1.1.1.1 proto tcp port 443 \
           /etc/lpf.conf";
        ];
      files = shared_files;
      safety_notes =
        [ "This command is read-only and does not inspect host state." ];
      see_also = [ "lpf-check(8)"; "lpf.conf(5)" ];
    };
    {
      command = Test;
      ...
      description =
        [
          "Run policy assertions covering decisions, NAT, route, queue, log, \
           and tables.";
        ];
      ...
      safety_notes =
        [ "Policy tests must run before guarded apply in CI workflows." ];
    ...
    };
    {
      ...
      description =
        [ "Manage dynamic policy tables without a full policy reload." ];
      options =
        [
    ...
    };
    {
      ...
      safety_notes =
        [ "Killing conntrack entries can interrupt active connections." ];
    ...
    };
    {
      ...
      description =
        [
          "Render deterministic read-only nftables rules from a checked lpf \
           policy.";
          "Diff rendered rules against observed lpf-owned nftables table \
           blocks.";
          "Observed rules can be supplied from a file/stdin or read live with \
           `nft list ruleset`.";
      ...
      options =
        [
          ( "--backend nftables",
            "select nftables rendering; currently the only backend" );
          ( "--observed <ruleset>",
            "read observed nftables ruleset text from a file or - for stdin" );
          ( "--live",
            "read observed nftables ruleset text with `nft list ruleset`" );
        ];
      examples =
        [
    ...
    };
    {
      ...
      description =
        [
          "Show policy apply history, checksums, tests, operators, and \
           rollback points.";
        ];
    ...
    };
    {
      ...
      description =
        [ "Generate, check, or install man pages from OCaml command metadata." ];
      options =
        [
          ( "--dir <path>",
            "select generated man page directory for generate/check" );
      ...
      safety_notes =
        [
          "Generated man pages must be committed with command behavior changes.";
        ];
    ...
    };
    {
      ...
      description =
        [
          "Emit JSON tool-calling schemas for automation agents that need to \
           call lpf commands.";
          "The output is generated from the same OCaml command metadata used \
           for help text and man pages.";
        ];
      options =
        [
          ( "--format openai",
            "emit OpenAI-style tool schema objects; this is the default" );
          ("--format jsonschema", "emit standalone JSON Schema objects");
          ( "--format system-prompt",
            "emit a JSON string containing a compact lpf automation prompt" );
        ];
      examples =
        [
          "lpf tools";
      ...
      safety_notes =
        [
          "This command is read-only and must not include host inventory or \
           credentials.";
        ];
    ...
    };
    {
      ...
      description =
        [
          ...
          "check mode reads required sysctls from /proc/sys and prints \
           key=value pairs.";
          "diff mode snapshots observed sysctls and diffs them against the \
           required set.";
        ];
      options = [];
