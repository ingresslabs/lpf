...
let write key value =
  ...
  if Sys.file_exists path then
    ...
    with e ->
      Error
        (Printf.sprintf "failed to write sysctl %s: %s" key
...
let check_required () =
  List.filter_map
    (fun key ->
      match read key with Ok value -> Some { key; value } | Error _ -> None)
    (required_sysctls ())
...
let diff ~intended ~observed =
  let intended_map =
    List.fold_left (fun acc e -> (e.key, e.value) :: acc) [] intended
  in
  let observed_map =
    List.fold_left (fun acc e -> (e.key, e.value) :: acc) [] observed
  in
  let changes = ref false in
  let buf = Buffer.create 256 in
  List.iter
    (fun (key, intended_value) ->
      ...
      | None ->
          ...
          Buffer.add_string buf
            (Printf.sprintf "+%s=%s (missing)\n" key intended_value)
      | _ -> ())
    intended_map;
  List.iter
    (fun (key, observed_value) ->
      if not (List.mem_assoc key intended_map) then (
        changes := true;
        Buffer.add_string buf
          (Printf.sprintf "-%s=%s (extra)\n" key observed_value)))
    observed_map;
...
let entry_json (e : entry) =
  Json_util.field_object
    [ ("key", Json_util.string e.key); ("value", Json_util.string e.value) ]

let to_json (entries : t) = Json_util.list entry_json entries ^ "\n"
