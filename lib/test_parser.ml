                ...
                [
                  {
                    ...
                    span = { file; line = 0; column = 1; end_column = 1 };
                    message = "expect outside of test case";
                  };
                ]
          | Some case ->
              ...
              let rec parse_expect expect = function
                ...
                | "in" :: rest ->
                    parse_expect
                      {
                        expect with
                        packet = { expect.packet with direction = Policy.In };
                      }
                      rest
                | "out" :: rest ->
                    parse_expect
                      {
                        expect with
                        packet = { expect.packet with direction = Policy.Out };
                      }
                      rest
                | "on" :: iface :: rest ->
                    parse_expect
                      {
                        expect with
                        packet = { expect.packet with interface = iface };
                      }
                      rest
                | "from" :: addr :: rest ->
                    parse_expect
                      {
                        expect with
                        packet = { expect.packet with source = addr };
                      }
                      rest
                | "to" :: addr :: rest ->
                    parse_expect
                      {
                        expect with
                        packet = { expect.packet with destination = addr };
                      }
                      rest
                | "proto" :: proto :: rest ->
                    let p =
                      if proto = "any" then Policy.Proto_any
                      else Policy.Proto_named proto
                    in
                    parse_expect
                      {
                        expect with
                        packet = { expect.packet with protocol = p };
                      }
                      rest
                | "port" :: port :: rest ->
                    let p = int_of_string_opt port in
                    parse_expect
                      { expect with packet = { expect.packet with port = p } }
                      rest
              ...
              loop suite
                (Some
                   { case with expectations = case.expectations @ [ expect ] })
                rest
