        ...
        List.filter_map
          (fun (q : Ir.queue) ->
            ...
            | Some classid ->
                ...
                | None -> None
                | Some parent ->
                    Some (Class_add { device; classid; parent; kind = "htb"; rate = q.bandwidth }))
          device_queues
      in
      qdisc :: classes)
    queues_by_device
...
let diff ~intended ~observed_qdisc ~observed_class =
  let obs_qdisc_map =
    List.fold_left (fun acc (q : observed_qdisc) ->
      (q.device, q) :: acc) [] observed_qdisc in
  let obs_class_map =
    List.fold_left (fun acc (c : observed_class) ->
      (c.device ^ ":" ^ c.classid, c) :: acc) [] observed_class in
  ...
  List.iter (function
    | Qdisc_add qd ->
        ...
        (match List.assoc_opt key obs_qdisc_map with
         | None ->
             changes := true;
             Buffer.add_string buf (Printf.sprintf "-qdisc dev %s: missing\n" key);
             Buffer.add_string buf (Printf.sprintf "+qdisc dev %s %s handle %s %s\n"
               qd.device qd.parent qd.handle qd.kind)
         | Some obs ->
             if not (String.equal qd.kind obs.kind && String.equal qd.handle obs.handle) then (
               changes := true;
               Buffer.add_string buf (Printf.sprintf "-qdisc dev %s: %s handle %s\n"
                 qd.device obs.kind obs.handle);
               Buffer.add_string buf (Printf.sprintf "+qdisc dev %s: %s handle %s\n"
                 qd.device qd.kind qd.handle)))
    | Class_add cl ->
        let key = cl.device ^ ":" ^ cl.classid in
        (match List.assoc_opt key obs_class_map with
         | None ->
             changes := true;
             Buffer.add_string buf (Printf.sprintf "-class dev %s classid %s: missing\n"
               cl.device cl.classid);
             Buffer.add_string buf (Printf.sprintf "+class dev %s parent %s classid %s %s rate %s\n"
               cl.device cl.parent cl.classid cl.kind cl.rate)
         | Some obs ->
             if not (String.equal cl.kind obs.kind && String.equal cl.classid obs.classid) then (
               changes := true;
               Buffer.add_string buf (Printf.sprintf "-class dev %s: %s classid %s parent %s rate %s\n"
