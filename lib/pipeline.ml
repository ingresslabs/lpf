...
let check_policy_text ?file text =
  ...
  | Some policy -> (
      ...
      | Ok ir ->
          ...
          {
            Policy.diagnostics = result.diagnostics @ shadow_diags;
            policy = Some policy;
          })
...
let render_tc_policy_text ?file text =
  match plan_policy_text ?file text with
  | Ok (plan, diagnostics) ->
      Ok (Tc.to_string (Tc.compile plan.policy), diagnostics)
  | Error diagnostics -> Error diagnostics
...
let diff_ebpf_policy ?file ~observed text =
  match plan_policy_text ?file text with
  | Ok (plan, diagnostics) ->
      let intended = Ebpf.render_plan plan in
      Ok
        ( Ebpf.diff ~intended ~observed,
          diagnostics @ Ebpf.capability_diagnostics plan.policy )
  | Error diagnostics -> Error diagnostics
...
let diff_nftables_policy_text ?file ~observed text =
  ...
  | Ok (intended, diagnostics) ->
      Ok (Nftables.diff_text ~intended ~observed, diagnostics)
  | Error diagnostics -> Error diagnostics

let diff_nftables_policy ?file ~observed text =
  match render_nftables_policy_text ?file text with
  | Ok (intended, diagnostics) ->
      Ok (Nftables.diff ~intended ~observed, diagnostics)
  | Error diagnostics -> Error diagnostics
  ...
  | Ok suite -> (
      let policy_path =
        if Filename.is_relative suite.policy_file then
          match file with
          | Some path ->
              Filename.concat (Filename.dirname path) suite.policy_file
