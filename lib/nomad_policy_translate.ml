...
let translate_one_group group_json =
  ...
  let name =
    match field_string "Name" fields with Some n -> n | None -> "unnamed"
  in
  ...
  List.iter
    (fun net_json ->
      ...
      let mode =
        match field_string "Mode" net_fields with
        | Some m -> m
        | None -> "bridge"
      in
      let ports =
        field_list "DynamicPorts" net_fields
        @ field_list "ReservedPorts" net_fields
      in
      ...
      if mode = "bridge" then (
        List.iter
          (fun port_json ->
            ...
            let port_num =
              match field_string "Value" port_fields with
              | Some s -> (
            ...
            if port_num > 0 then
              rules :=
                Printf.sprintf
                  "  pass in on eth0 proto tcp from any to any port %d" port_num
                :: !rules)
          ports;
...
let translate_nomad_network json =
  try
    ...
    let job_name =
      match field_string "Name" job_fields with
      | Some n -> n
      | None -> "unnamed"
    in
    let groups = field_list "TaskGroups" job_fields in
    let policy_text =
      String.concat "\n" (List.map translate_one_group groups)
    in
