...
let rule_list_invocation () =
  { program = "ip"; argv = [ "ip"; "rule"; "list" ] }

let route_show_invocation table =
  {
    program = "ip";
    argv = [ "ip"; "route"; "show"; "table"; string_of_int table ];
  }
...
let parse_rule_list output =
  ...
  |> List.filter_map (fun line ->
         ...
         if String.length line = 0 then None
           ...
           | priority_str :: rest -> (
               let priority =
                 match int_of_string_opt (String.trim priority_str) with
                 | Some p -> p
                 | None -> 0
               in
               ...
               let fwmark =
                 Option.bind (field_after "fwmark" parts) int_token
               in
  ...
  |> List.filter_map (fun line ->
         ...
         if String.length line = 0 then None
           ...
           | "default" :: rest ->
               ...
               Some
                 {
                   gateway = Option.value gateway ~default:"";
                   device;
                   table = 0;
                 }
           | _ -> (
               ...
               let subnet =
                 match parts with
                 | first :: _ when not (String.equal first "default") ->
                     Some first
                 | _ -> None
               in
...
let delete_rules_invocation () =
  { program = "ip"; argv = [ "ip"; "rule"; "flush" ] }

let delete_rules_with_runner runner =
  runner (delete_rules_invocation ()) |> Result.map ignore

let delete_rules () = delete_rules_with_runner run

let flush_table_invocation table =
  {
    program = "ip";
    argv = [ "ip"; "route"; "flush"; "table"; string_of_int table ];
  }

let flush_table_with_runner runner table =
  runner (flush_table_invocation table) |> Result.map ignore
