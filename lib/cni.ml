...
let parse_ipam json =
  ...
  let ipam_type = match List.assoc_opt "type" obj with
    | Some (String t) -> t | _ -> "host-local"
  in
  let ipam_subnet = match List.assoc_opt "subnet" obj with
  ...
  let ipam_routes =
    ...
    | Some (Array items) ->
      List.filter_map (fun item ->
        match item with
        | Object fields ->
          let dst = List.assoc_opt "dst" fields |> opt_bind string_value in
...
let parse_policy json =
  ...
  let obj = match json with Object o -> o | _ -> [] in
  {
    policy_mode = (match List.assoc_opt "mode" obj |> opt_bind string_value with Some m -> m | None -> "auto");
    default_action = (match List.assoc_opt "defaultAction" obj |> opt_bind string_value with Some a -> a | None -> "deny");
    cluster_policy_ref = List.assoc_opt "clusterPolicyRef" obj |> opt_bind string_value;
    log_dropped = (match List.assoc_opt "logDropped" obj |> opt_bind bool_value with Some b -> b | None -> true);
    audit_mode = (match List.assoc_opt "auditMode" obj |> opt_bind bool_value with Some b -> b | None -> false);
  }
...
let parse_network_config text =
  ...
  | Ok json ->
    let obj = match json with Object o -> o | _ -> [] in
    let cni_version = match List.assoc_opt "cniVersion" obj |> opt_bind string_value with
      | Some v -> v | None -> "0.3.1"
    in
    let name = match List.assoc_opt "name" obj |> opt_bind string_value with
      | Some n -> n | None -> "lpf"
    in
    let cnitype = match List.assoc_opt "type" obj |> opt_bind string_value with
      | Some t -> t | None -> "lpf-cni"
    in
    let ipam = match List.assoc_opt "ipam" obj with
      | Some ipam_json -> Some (parse_ipam ipam_json)
      | None -> None
    in
    let policy = match List.assoc_opt "policy" obj with
...
let create_veth_pair host_if container_if =
  run_cmd (Printf.sprintf "ip link add %s type veth peer name %s" host_if container_if)
...
let set_if_up ifname =
  run_cmd (Printf.sprintf "ip link set %s up" ifname)

let set_if_up_in_netns_by_path ifname netns_path =
  run_cmd (Printf.sprintf "nsenter --net=%s ip link set %s up" netns_path ifname)

let assign_ip ifname ip_addr netns_path =
  run_cmd (Printf.sprintf "nsenter --net=%s ip addr add %s dev %s" netns_path ip_addr ifname)

let add_route ifname dst gw netns_path =
  let gw_str = match gw with Some g -> " via " ^ g | None -> "" in
  let dev_str = " dev " ^ ifname in
  run_cmd (Printf.sprintf "nsenter --net=%s ip route add %s%s%s" netns_path dst gw_str dev_str)
  ...
  let ipam_input =
    let ipam_obj =
      ...
      (if ipam_config.ipam_routes <> [] then
         [("routes", Array (List.map (fun (dst, gw) ->
           let fields = [("dst", String dst)] @
             (match gw with Some g -> [("gw", String g)] | None -> [])
           in
           Object fields) ipam_config.ipam_routes))]
       else [])
    in
      ...
      | Ok ipam_json ->
        ...
        let ips =
          ...
          | Some (Array items) ->
            List.filter_map (fun item ->
              match item with
              | Object fields ->
                let addr = List.assoc_opt "address" fields |> opt_bind string_value in
                let gw = List.assoc_opt "gateway" fields |> opt_bind string_value in
              ...
              | _ -> None) items
          | _ ->
            begin match List.assoc_opt "ip4" obj with
            | Some (Object fields) ->
              let ip = List.assoc_opt "ip" fields |> opt_bind string_value in
              let gw = List.assoc_opt "gateway" fields |> opt_bind string_value in
              begin match ip with
              | Some i -> [(i, gw)]
              | None -> []
        ...
        let dns_servers =
          ...
          | Some (Object dns_fields) ->
            begin match List.assoc_opt "nameservers" dns_fields with
            | Some (Array items) ->
              List.filter_map string_value items
            | _ -> []
            end
          | _ -> []
        in
...
let find_cgroup_path container_id =
  ...
  | Ok pid_str ->
    ...
    if pid = "" then Error (Printf.sprintf "no process found for container %s" container_id)
    else
      let cgroup_cmd = Printf.sprintf "cat /proc/%s/cgroup 2>/dev/null | head -1 | cut -d: -f3" pid in
      begin match run_cmd_out cgroup_cmd with
      | Ok cgroup_rel ->
  ...
  | Error _e ->
    let netns_hint = Printf.sprintf "grep -l %s /proc/*/net/ns/net 2>/dev/null | head -1 | cut -d/ -f3" container_id in
    match run_cmd_out netns_hint with
    | Ok pid_str ->
      let pid = trim pid_str in
      if pid = "" then Error (Printf.sprintf "cannot find cgroup for container %s" container_id)
      else
        let cgroup_cmd = Printf.sprintf "cat /proc/%s/cgroup 2>/dev/null | head -1 | cut -d: -f3" pid in
        begin match run_cmd_out cgroup_cmd with
        | Ok cgroup_rel ->
    ...
    | Error _ -> Error (Printf.sprintf "cannot find cgroup for container %s" container_id)
...
let attach_bpf cgroup_path _policy_mode =
  ...
  if Sys.file_exists prog_ingress then
    ignore (run_cmd (Printf.sprintf "bpftool cgroup attach %s cgroup_skb ingress pinned %s 2>/dev/null" cgroup_path prog_ingress));
  if Sys.file_exists prog_egress then
    ignore (run_cmd (Printf.sprintf "bpftool cgroup attach %s cgroup_skb egress pinned %s 2>/dev/null" cgroup_path prog_egress))
...
let detach_bpf cgroup_path =
  ...
  let prog_egress = Filename.concat bpffs "progs/cgroup_egress" in
  if Sys.file_exists prog_ingress then
    ignore (run_cmd (Printf.sprintf "bpftool cgroup detach %s cgroup_skb ingress pinned %s 2>/dev/null" cgroup_path prog_ingress));
  if Sys.file_exists prog_egress then
    ignore (run_cmd (Printf.sprintf "bpftool cgroup detach %s cgroup_skb egress pinned %s 2>/dev/null" cgroup_path prog_egress))

(* ─── ADD handler ─── *)

let handle_add cfg netns_path container_id ifname =
  let host_if, container_if = veth_name container_id ifname in
  begin match create_veth_pair host_if container_if with
  ...
  | Ok () ->
    ...
    | Ok () ->
      ...
      | Ok () ->
        ...
        | Ok () ->
          begin match cfg.ipam with
          | Some ipam_cfg ->
            let cni_path = getenv_default "CNI_PATH" "/opt/cni/bin" in
            begin match call_ipam cni_path ipam_cfg container_id container_if with
            | Error e ->
              ignore (delete_veth host_if);
            ...
            | Ok (ip_addr, gw, routes, dns_servers) ->
              ...
              begin match assign_ip container_if ip_addr netns_path with
              | Error e ->
                ignore (delete_veth host_if);
                Error (Printf.sprintf "ip assign: %s" e)
              | Ok () ->
                match add_default_route container_if gw netns_path with
                | Error e ->
                  ignore (delete_veth host_if);
                ...
                | Ok () ->
                  let rec add_routes = function
                    | [] -> Ok ()
                    | (dst, dst_gw) :: rest ->
                      match add_route container_if dst dst_gw netns_path with
                      | Error e -> Error (Printf.sprintf "route %s: %s" dst e)
                      | Ok () -> add_routes rest
                  in
                  ...
                  | Error e ->
                    ...
                    Error e
                  | Ok () ->
                    begin match cfg.policy with
                    | Some _policy ->
                      begin match find_cgroup_path container_id with
                      | Ok cg_path ->
                        attach_bpf cg_path "auto";
                        Ok { ip_address = ip_addr; gateway = gw; routes = routes; dns_nameservers = dns_servers }
                      | Error _ ->
                        Ok { ip_address = ip_addr; gateway = gw; routes = routes; dns_nameservers = dns_servers }
                      end
                    | None ->
          ...
          | None ->
            Ok { ip_address = ""; gateway = None; routes = []; dns_nameservers = [] }
          end
  end
...
let handle_del cfg _netns_path container_id ifname =
  ...
  begin match cfg.ipam with
  | Some _ipam_cfg ->
    ignore (run_cmd (Printf.sprintf "ip addr flush dev %s 2>/dev/null" host_if))
  | None -> ()
  end;
  ignore (delete_veth host_if);
  Ok ()
...
let handle_version () =
  let version_json = Json_parse.string_of_json (Json_parse.Object [
    ("cniVersion", Json_parse.String "1.0.0");
    ("supportedVersions", Json_parse.Array [
...
let result_to_json result =
  let open Json_parse in
  let ips = Array [Object [
    ("address", String result.ip_address);
    ("gateway", match result.gateway with Some g -> String g | None -> Null);
    ("interface", String "eth0");
  ]] in
  let routes = Array (List.map (fun (dst, gw) ->
    let fields = [("dst", String dst)] in
    let fields = match gw with Some g -> ("gw", String g) :: fields | None -> fields in
    Object fields
  ) result.routes) in
  let dns = Object [
    ("nameservers", Array (List.map (fun s -> String s) result.dns_nameservers))
  ] in
  string_of_json (Object [
