...
let list_invocation () =
  { program = "conntrack"; argv = [ "conntrack"; "-L"; "-o"; "extended" ] }

let delete_invocation ~src ~dst ?sport ?dport () =
  let args = [ "conntrack"; "-D"; "-s"; src; "-d"; dst ] in
  let args =
    match sport with
    | Some sp -> args @ [ "-p"; "tcp"; "--orig-port-src"; sp ]
    | None -> args
  in
  let args =
    match dport with
    | Some dp -> args @ [ "-p"; "tcp"; "--orig-port-dst"; dp ]
    | None -> args
  in
  { program = "conntrack"; argv = args }

let flush_invocation () =
  { program = "conntrack"; argv = [ "conntrack"; "-F" ] }
...
let delete_with_runner runner ~src ~dst ?sport ?dport () =
  runner (delete_invocation ~src ~dst ?sport ?dport ()) |> Result.map ignore

let delete ~src ~dst ?sport ?dport () =
  delete_with_runner run ~src ~dst ?sport ?dport ()
...
let parse_line line =
  ...
  let rec find_field_starts prefix = function
    | field :: _
      when String.starts_with ~prefix field || String.equal field prefix -> (
  ...
  | proto :: src :: dst :: rest ->
      let sport =
        Option.value (find_field "sport=" rest)
          ~default:(Option.value (find_field "sport" rest) ~default:"")
      in
      let dport =
        Option.value (find_field "dport=" rest)
          ~default:(Option.value (find_field "dport" rest) ~default:"")
      in
      let state =
        Option.value
          (find_field_starts "TIME_WAIT" rest)
          ~default:
            (Option.value
               (find_field_starts "ESTABLISHED" rest)
               ~default:
                 (Option.value
                    (find_field_starts "CLOSE" rest)
                    ~default:
                      (Option.value
                         (find_field_starts "SYN_SENT" rest)
                         ~default:
                           (Option.value
                              (find_field_starts "NONE" rest)
                              ~default:
                                (Option.value
                                   (find_field_starts "ASSURED" rest)
                                   ~default:
                                     (Option.value
                                        (find_field_starts "UNREPLIED" rest)
                                        ~default:
                                          (match
                                             find_field "[UNREPLIED]" rest
                                           with
                                          | Some _ -> "UNREPLIED"
                                          | None -> "")))))))
      in
  ...
  | _ -> (
      ...
      | proto :: rest when String.ends_with ~suffix:"src=" proto ->
          let src =
            match String.split_on_char '=' proto with
            | [ _; v ] -> v
            | _ -> proto
          in
          ...
          Some
            {
              protocol =
                (match find_field "protonum=" rest with
                | Some p -> p
                | None -> "unknown");
              src;
              dst;
...
let parse_list output =
  String.split_on_char '\n' output |> List.filter_map parse_line
...
let entries_to_json entries = Json_util.list entry_to_json entries ^ "\n"
