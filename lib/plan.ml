...
let of_ir policy = { schema; checksum = Ir_json.checksum_of_ir schema policy; policy }
...
let to_json plan =
  Json_util.field_object
    [
      ...
      ("policy", Ir_json.policy_json ~include_spans:true ~for_checksum:false plan.policy);
    ]
  ^ "\n"
