...
let field_obj name fields =
  match field name fields with
  | Some (Json_parse.Object obj) -> obj
  | _ -> []
...
let pod_selector_to_set_name json =
  ...
  | Some (Json_parse.Object labels) ->
    let pairs = List.filter_map (fun (k, v) ->
      match v with Json_parse.String s -> Some (k, s) | _ -> None) labels in
...
let translate_ports ports =
  if ports = [] then " any"
    ...
    let port_strs = List.filter_map (fun port_json ->
      let fields = obj_fields port_json in
      let port = match field "port" fields with
        | Some (Json_parse.Number f) -> Some (int_of_float f)
        | Some (Json_parse.String s) -> begin match int_of_string_opt s with Some n -> Some n | None -> None end
        | _ -> None
      in
      let end_port = match field "endPort" fields with
        | Some (Json_parse.Number f) -> Some (int_of_float f)
        | _ -> None
      in
      let protocol = match field_string "protocol" fields with
        | Some p -> protocol_string p
        | None -> "tcp"
      in
      match port, end_port with
      | Some p, Some ep -> Some (Printf.sprintf " proto %s port %d:%d" protocol p ep)
...
let translate_peer to_json =
  ...
  | None, None ->
    ...
    | Some (Json_parse.Object ib_fields) ->
      begin match field_string "cidr" ib_fields with
      | Some cidr ->
        let except = field_list "except" ib_fields in
        if except = [] then
          Some (Printf.sprintf " from %s" cidr)
        else
          Some (Printf.sprintf " from %s" cidr)
      | None -> Some " from any"
      end
...
let translate_ingress_rule rule_json =
  ...
  if froms = [] then
    ["pass in on eth0 proto tcp from any to any" ^ port_str]
  else
    List.filter_map (fun from_json ->
      match translate_peer from_json with
      | Some peer_str -> Some ("pass in on eth0" ^ peer_str ^ " to any" ^ port_str)
      | None -> None
    ) froms
...
let translate_egress_rule rule_json =
  ...
  if tos = [] then
    ...
    List.filter_map (fun to_json ->
      match translate_peer to_json with
      | Some peer_str -> Some ("pass out on eth0 from any" ^ peer_str ^ port_str ^ " keep state")
      | None -> None
    ) tos
...
let translate_one_np np_json =
  ...
  let name = match field_string "name" metadata with Some n -> n | None -> "unnamed" in
  let namespace = match field_string "namespace" metadata with Some n -> n | None -> "default" in
  ...
  begin
    if List.mem "Ingress" policy_types then begin
      let ingress_rules = field_list "ingress" spec in
      List.iter (fun rule ->
        rules := !rules @ translate_ingress_rule rule
      ) ingress_rules
    end;

    if List.mem "Egress" policy_types then begin
      let egress_rules = field_list "egress" spec in
      List.iter (fun rule ->
        rules := !rules @ translate_egress_rule rule
      ) egress_rules
    end
  end;
