...
|}
          ...
          | Ok json -> (
              ...
              | Ok policy -> (
                  require
                    (contains policy "pass out on eth0")
                    "policy contains pass out";
                  require
                    (contains policy "port 443")
                    "policy contains port 443";
                  require
                    (contains policy "keep state")
                    "policy contains keep state";
...
|}
                  ...
                  | Ok json -> (
                      ...
                      | Ok policy -> (
                          require
                            (contains policy "pass in on eth0")
                            "policy contains pass in";
                          require (contains policy "from <")
                            "policy contains from <set>";
                          require
                            (contains policy "port 5432")
                            "policy contains port 5432";
...
|}
                          ...
                          | Ok json -> (
                              ...
                              | Ok policy -> (
                                  require
                                    (contains policy "port 30000:32767")
                                    "policy contains port range";
...
|}
                                  ...
                                  | Ok json -> (
                                      match translate_network_policy json with
                                      | Error e ->
                                          failwith ("translate error: " ^ e)
                                      | Ok policy ->
                                          require (contains policy "anchor")
                                            "policy contains anchor";
                                          Printf.printf
                                            "test5 default deny: OK\n%!";

                                          Printf.printf
                                            "network_policy_translate tests \
                                             passed\n")))))))))
