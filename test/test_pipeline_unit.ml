let () =
  let result = Lpf.render_tc_policy_text "set default deny\ninterface wan = \"eth0\"\nqueue std on wan bandwidth 10M\npass out on wan from any to any queue std" in
  (match result with
  ...
  let result = Lpf.render_routing_policy_text "set default deny\ninterface wan = \"eth0\"\npass out on wan proto tcp from any to any port 443 route-to 1.1.1.1 (wan) keep state" in
  (match result with
   | Ok (rendered, diagnostics) ->
       assert (diagnostics = []);
       assert (String.length rendered > 0);
  ...
  let policy_text = "set default deny\ninterface wan = \"eth0\"\nqueue std on wan bandwidth 10M\ntable <trusted> { 10.0.0.0/8 }\npass in on wan from <trusted> to any queue std\npass out proto tcp from any to any port 443 keep state" in
