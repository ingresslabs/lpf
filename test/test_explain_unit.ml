...
let () =
  let ir_out_pass = ir_of_text "set default deny\npass out proto tcp from any to any port 443" in

  let tcp_http_out = { packet with direction = Lpf.Policy.Out; protocol = Lpf.Policy.Proto_named "tcp"; port = Some 443 } in
  ...
  let tcp_http_in = { packet with protocol = Lpf.Policy.Proto_named "tcp"; port = Some 443 } in
  let result = Lpf.Explain.explain ir_out_pass tcp_http_in in
  assert (result.decision = Lpf.Policy.Block);

  let tcp_ssh_in = { packet with protocol = Lpf.Policy.Proto_named "tcp"; port = Some 22 } in
  let result = Lpf.Explain.explain ir_out_pass tcp_ssh_in in
  assert (result.decision = Lpf.Policy.Block);

  let ir_pass_block_icmp = ir_of_text "set default pass\nblock in proto icmp from any to any" in
  ...
  let udp_in = { packet with protocol = Lpf.Policy.Proto_named "udp"; port = Some 53 } in
  let result = Lpf.Explain.explain ir_pass_block_icmp udp_in in
  assert (result.decision = Lpf.Policy.Pass);

  let ir_pass_ssh = ir_of_text "set default deny\npass in proto tcp from any to any port 22 keep state" in
  let tcp_ssh_in = { packet with protocol = Lpf.Policy.Proto_named "tcp"; port = Some 22 } in
