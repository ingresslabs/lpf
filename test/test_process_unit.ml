let () =
  ...
  let failed_msg = Lpf.Process.string_of_run_status (Lpf.Process.Failed_to_start "ENOENT") in
  require (String.contains failed_msg 'E') "failed_to_start should contain ENOENT";

  let invocation : Lpf.Process.invocation =
    { program = "test"; argv = [ "test"; "arg1"; "arg2" ] }
  in
  let error : Lpf.Process.run_error =
    { invocation; status = Lpf.Process.Exited 1; stderr = "something went wrong" }
  in
  ...
  let echo_result = Lpf.Process.run ~temp_prefix:"echo-test"
  ...
  let nonexist_result = Lpf.Process.run ~temp_prefix:"nonexistent-test"
    { program = "nonexistent-command-lpf-test"; argv = [ "nonexistent-command-lpf-test" ] }
  in
