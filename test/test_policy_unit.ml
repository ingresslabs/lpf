...
let () =
  ...
  assert (policy.interfaces |> List.hd |> fun i -> String.equal i.Lpf.Policy.device "eth0");
  let policy = assert_parse "set default deny\ntable <t> { 10.0.0.0/8, 192.168.0.0/16 }\npass in from <t> to any" in
  assert (List.length policy.tables = 1);
  assert (policy.tables |> List.hd |> fun t -> List.length t.Lpf.Policy.entries = 2)
