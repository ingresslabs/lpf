let () =
  ...
  (match result with Ok out -> assert (String.length out > 0) | _ -> assert false);
  ...
  let error = {
    Lpf.Conntrack.invocation = { Lpf.Conntrack.program = "conntrack"; argv = [ "conntrack"; "-L" ] };
    status = Lpf.Conntrack.Exited 1; stderr = "failed";
  } in
