let () =
  ...
  let diag = Lpf.Apply_guard.error_diagnostic ~file:"test.lpf" "something went wrong" in
  ...
  let mock_apply_fails = { mock_runners with apply = (fun _ -> Error {
    Lpf.Nft.invocation = { Lpf.Nft.program = "nft"; argv = [ "nft"; "-f"; "x" ] };
    status = Lpf.Nft.Exited 1; stderr = "apply failed";
  })} in
  let result =
  ...
  let result =
    Lpf.Apply_guard.apply_policy_text_with_runners mock_runners
      ~file:"test.lpf" ~confirm:"invalid" "set default deny\npass out proto tcp from any to any port 443"
  in
  (match result with
   | Error diagnostics -> assert (List.length diagnostics > 0)
   | _ -> assert false);
