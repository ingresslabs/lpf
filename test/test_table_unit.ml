let () =
  ...
  in
  let result =
    Lpf.Table.replace_with_runner track_runner "threats"
      [ "10.0.0.1"; "10.0.0.2" ]
  in
  assert (result = Ok ());
  assert (List.length !calls >= 3);

  let fails_first _invocation =
    Error
      {
  ...
  assert (
    List.exists
      (fun (element : Lpf.Table.table_element) ->
        String.equal element.value "10.0.0.1"
        && element.packets = Some 3 && element.bytes = Some 252)
      elements);
