let () =
  ...
  require
    (Sys.file_exists test_dir && Sys.is_directory test_dir)
    "ensure_dir should create directory";

  Lpf.File_util.ensure_dir ~strict:true test_dir;
  require
    (Sys.is_directory test_dir)
    "ensure_dir on existing directory should be ok";
  ...
  require
    ...
    "ensure_dir should create nested dirs";
  ...
  (try
     Lpf.File_util.ensure_dir ~strict:true test_file_in_place;
     failwith "should have raised"
   with Failure _ -> ());
  Lpf.File_util.ensure_dir ~strict:false test_file_in_place;
  (try
     Sys.remove test_file_in_place;
     Sys.remove lenient_dir
   with _ -> ());
