...
let assert_check_fails ~contains path =
  ...
  | { policy = Some _; diagnostics = _ } ->
      failwith (path ^ " unexpectedly passed")
  ...
  | { Lpf.Policy.policy = Some _; diagnostics } ->
      ...
      then
        failwith
          ...
          ^ String.concat "\n"
              (List.map Lpf.Policy.diagnostic_to_string diagnostics))
  | { policy = None; diagnostics = _ } ->
      failwith (path ^ " unexpectedly failed")
  ...
  | { Lpf.Policy.policy = None; diagnostics } ->
      assert (
        List.exists
          ...
          diagnostics)
  | { policy = Some _; diagnostics = _ } ->
      failwith (file ^ " unexpectedly passed")
...
let () =
  ...
  assert_check_has_diagnostic ~line:5 ~column:7
    ~message:"invalid queue name `bad/name`" "invalid-queue-name.lpf";
  ...
  assert_check_has_diagnostic ~line:7 ~column:8
    ~message:"duplicate anchor `office`" "invalid-anchor-duplicate.lpf";
  assert_check_has_diagnostic ~line:3 ~column:8
    ~message:"invalid anchor name `bad/name`" "invalid-anchor-name.lpf";
  ...
  assert_check_has_diagnostic ~line:4 ~column:44
    ~message:"rule references unknown queue `missing-queue`"
    "invalid-anchor-unknown-reference.lpf";
  assert_check_has_diagnostic ~line:3 ~column:1 ~message:"expected closing `}`"
    "invalid-anchor-unclosed.lpf";
  assert_check_has_diagnostic ~line:3 ~column:17
    ~message:"interface device must be quoted" "invalid-interface-device.lpf";
  assert_check_has_diagnostic ~line:3 ~column:17
    ~message:"unterminated quoted string" "invalid-quoted-string.lpf";
  assert_check_has_diagnostic ~line:3 ~column:29 ~message:"empty table entry"
  ...
  assert_check_has_diagnostic ~line:5 ~column:43
    ~message:"invalid rdr: invalid port `70000`" "invalid-rdr-port.lpf";
  ...
  (match
     Lpf.format_policy_text ~file:(fixture "queue-route.lpf") queue_route
   with
  ...
  assert_formats_to
    ~expected:
      ...
      \  pass in on lan from <trusted> to any queue std\n\
       ...
       pass out log (all) on wan proto tcp from any to any port $wan_port \
