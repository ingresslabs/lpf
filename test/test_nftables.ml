...
let nft_observed_fixture path =
  Filename.concat "../fixtures/nftables-observed" path
...
let assert_nftables_diff policy_name observed_path expected_name =
  ...
  | Ok (diff, diagnostics) ->
      ...
      if not (String.equal diff expected) then
        failwith
          ("nftables diff mismatch for " ^ policy_name ^ " / " ^ expected_name)
  | Error diagnostics ->
      failwith
        (String.concat "\n"
           (List.map Lpf.Policy.diagnostic_to_string diagnostics))
...
let assert_nftables_diff_fixtures () =
  ...
  assert_nftables_diff "basic.lpf"
    (nft_observed_fixture "missing-owned.nft")
    "missing-owned.diff";
  assert_nftables_diff "basic.lpf"
    (nft_observed_fixture "extra-rule.nft")
    "extra-rule.diff";
  assert_nftables_diff "basic.lpf"
    (nft_observed_fixture "changed-rule.nft")
    "changed-rule.diff"
...
let assert_nftables_structured_diff_fixtures () =
  ...
  (match
     Lpf.diff_nftables_policy ~file:path
       ~observed:(read_file (nft_fixture "basic.nft"))
       policy
   with
  | Ok (diff, diagnostics) ->
      assert (diagnostics = []);
      ...
      assert (
        String.equal diff.text (read_file (nft_diff_fixture "unchanged.diff")))
  | Error diagnostics ->
      failwith
        (String.concat "\n"
           (List.map Lpf.Policy.diagnostic_to_string diagnostics)));
  ...
  | Ok (diff, diagnostics) ->
      ...
      assert (
        String.equal diff.text
          (read_file (nft_diff_fixture "changed-rule.diff")))
  | Error diagnostics ->
      failwith
        (String.concat "\n"
           (List.map Lpf.Policy.diagnostic_to_string diagnostics))
...
let assert_live_nftables_readback () =
  ...
  let path = fixture "basic.lpf" in
  match
    Lpf.diff_nftables_policy_text ~file:path ~observed:live_observed
      (read_file path)
  with
  | Ok (diff, diagnostics) ->
      assert (diagnostics = []);
