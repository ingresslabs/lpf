...
interfaces, macros, tables, queues, pass/block rules, NAT, redirects, and
route-to annotations with OCaml tests and valid/invalid fixtures. The parser is
tokenized and carries column-level spans for actionable diagnostics. The typed
IR, backend plan generation, and networking backend behavior are intentionally
not implemented yet.
