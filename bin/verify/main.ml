let () =
  let usage_msg = "lpf-verify - formally verify lpf policy properties with Z3\n\n\
  ...
  let speclist = [
    ...
    ("--second", Arg.String set_second_file, "FILE Second policy file (for equivalence)");
    ("-s", Arg.String set_second_file, "FILE Second policy file");
    ("--action", Arg.String set_action, "ACTION Target action: pass/block/reject");
    ("-a", Arg.String set_action, "ACTION Target action");
    ("--constraint", Arg.String add_constraint, "K=V Constraint (src=10.0.0.1,dst=10.0.0.2,port=443,proto=tcp)");
  ...
  let cmd = match args with
    | [] ->
      ...
      exit 64
    | cmd :: rest ->
      if policy_file = None then begin match rest with
        | f :: _ -> policy_file := Some f
        | _ -> ()
      end;
      cmd
  in
  ...
  let parse_ir text =
    ...
    | Error diagnostics ->
      ...
      List.iter (fun (d : Lpf.Policy.diagnostic) ->
        Printf.eprintf "  %s\n" d.message) diagnostics;
      exit 1
  in
  ...
  let parse_constraints () =
    ...
    List.iter (fun c ->
      ...
      List.iter (fun pair ->
        ...
        | _ -> Printf.eprintf "warning: ignoring malformed constraint: %s\n" pair
      ) parts
    ) !constraints;
    List.rev !cls
  in
  ...
  let parse_invariants () =
    ...
    List.iter (fun inv ->
      ...
      List.iter (fun pair ->
        match String.split_on_char '=' pair with
        | [field; value] ->
          let clause = Lpf_z3.Z3_verify.(Field (String.trim field, "==", String.trim value)) in
          invs := clause :: !invs
        | _ -> Printf.eprintf "warning: ignoring malformed invariant: %s\n" pair
      ) parts
    ) !invariants;
    List.rev !invs
  in
  ...
  match cmd with
  | "consistency" ->
    let file = match !policy_file with Some f -> f | None -> Printf.eprintf "no policy file\n"; exit 64 in
    ...
    if dead_rules = [] then begin
      Printf.printf "PASS: no dead or shadowed rules found\n";
      exit 0
    end else begin
      Printf.printf "FOUND %d dead/shadowed rule(s):\n" (List.length dead_rules);
      List.iter (fun (dr : Lpf_z3.Z3_verify.dead_rule) ->
        Printf.printf "  rule at line %d is shadowed: %s\n" dr.line dr.reason) dead_rules;
      exit 1
    end

  | "equivalence" ->
    let file1 = match !policy_file with Some f -> f | None -> Printf.eprintf "no first policy\n"; exit 64 in
    let file2 = match !second_file with Some f -> f | None -> Printf.eprintf "no second policy (use -s)\n"; exit 64 in
    ...
    | Lpf_z3.Z3_verify.Not_equivalent ne ->
      ...
      Printf.printf "  counterexample: src=%s dst=%s port=%d decision1=%s decision2=%s\n"
  ...
  | "reachable" ->
    let file = match !policy_file with Some f -> f | None -> Printf.eprintf "no policy file\n"; exit 64 in
    let action_str = match !target_action with Some a -> a | None -> Printf.eprintf "no target action (use -a)\n"; exit 64 in
    let target_action = match String.lowercase_ascii action_str with
      | "pass" -> Lpf.Ir.Pass | "block" -> Lpf.Ir.Block | "reject" -> Lpf.Ir.Reject
      | _ -> Printf.eprintf "unknown action: %s (use pass/block/reject)\n" action_str; exit 64
    in
    ...
    match Lpf_z3.Z3_verify.check_reachable ir ~constraints:cls ~target_action with
    | Lpf_z3.Z3_verify.Reachable ce ->
      Printf.printf "REACHABLE: packet exists that matches constraints and produces %s\n" action_str;
      Printf.printf "  src=%s dst=%s port=%d\n" ce.source ce.destination
        (match ce.port with Some p -> p | None -> 0);
      exit 0
    | Lpf_z3.Z3_verify.Unreachable ->
      Printf.printf "PASS: no packet matches constraints and produces %s (unreachable)\n" action_str;
      exit 0
  ...
  | "invariant" ->
    ...
    if invs = [] then begin
      Printf.eprintf "no invariants specified (use -i src=10.0.0.0/8 -i dst=0.0.0.0/0)\n";
      exit 64
    end;
  ...
  | "coverage" ->
    ...
    List.iter (fun (rc : Lpf_z3.Z3_verify.rule_coverage) ->
      Printf.printf "  rule at line %d (%s): %s\n" rc.line rc.action
        (if rc.reachable then "reachable" else "DEAD (never matched)")
    ) coverage
  ...
  | "gen-tests" ->
    ...
    List.iter (fun (t : Lpf_z3.Z3_verify.generated_test) ->
      Printf.printf "test \"%s\" { expect = %s }\n" t.test_name t.expected_action) tests

  | "backend-ebpf" ->
    let file = match !policy_file with Some f -> f | None -> Printf.eprintf "no policy file\n"; exit 64 in
    let ir = parse_ir (read_policy file) in
    Printf.printf "checking eBPF backend equivalence for %s...\n%!" file;
    let ebpf_filter = Lpf.Ebpf.classify ir in
    match Lpf_z3.Z3_verify.check_backend_equivalence ~ir ~ebpf_rules:ebpf_filter with
    | Lpf_z3.Z3_verify.Equivalent ->
      Printf.printf "PASS: eBPF backend is semantically equivalent to IR\n";
      exit 0
    | Lpf_z3.Z3_verify.Not_equivalent ne ->
      Printf.printf "FAIL: eBPF backend diverges from IR\n";
      Printf.printf "  counterexample: src=%s dst=%s decision1=%s decision2=%s\n"
        ne.counterexample.source ne.counterexample.destination
        ne.decision_in_first ne.decision_in_second;
      exit 1
  ...
  | "check-all" ->
    ...
    if dead = [] then Printf.printf "  PASS\n"
      ...
      List.iter (fun d -> Printf.printf "    rule at line %d: %s\n" d.Lpf_z3.Z3_verify.line d.Lpf_z3.Z3_verify.action) dead
    end;

    Printf.printf "\n[2/4] rule coverage...\n%!";
    let coverage = Lpf_z3.Z3_verify.check_rule_coverage ir in
    let unreachable = List.filter (fun rc -> not rc.reachable) coverage in
    if unreachable = [] then Printf.printf "  PASS: all rules reachable\n"
    else begin
      failed := true;
    ...
    Printf.printf "\n[3/4] minimize...\n%!";
    let minimal, removed = Lpf_z3.Z3_verify.minimize ir in
    if removed = 0 then Printf.printf "  PASS: no redundant rules\n"
    else Printf.printf "  PASS: %d redundant rules can be removed\n" removed;

    Printf.printf "\n[4/4] backend equivalence (eBPF)...\n%!";
    let ebpf_filter = Lpf.Ebpf.classify ir in
    begin match Lpf_z3.Z3_verify.check_backend_equivalence ~ir ~ebpf_rules:ebpf_filter with
    | Lpf_z3.Z3_verify.Equivalent -> Printf.printf "  PASS: eBPF backend matches IR\n"
    | Lpf_z3.Z3_verify.Not_equivalent ne ->
      failed := true;
      Printf.printf "  FAIL: divergence at src=%s dst=%s\n"
        ne.counterexample.Lpf_z3.Z3_verify.source ne.counterexample.Lpf_z3.Z3_verify.destination
    end;
