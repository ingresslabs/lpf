let () =
  ...
  let parse_ir text =
    let parsed = Lpf.Policy.parse text in
    match parsed.policy with
    | None ->
        Printf.eprintf "lpf-verify: policy parse error\n";
        List.iter
          (fun (d : Lpf.Policy.diagnostic) ->
            Printf.eprintf "  %s\n" (Lpf.Policy.diagnostic_to_string d))
          parsed.diagnostics;
        exit 1
    | Some policy -> (
        ...
        | Ok ir -> ir
        | Error diagnostics ->
            Printf.eprintf "lpf-verify: policy validation error\n";
            List.iter
              (fun (d : Lpf.Policy.diagnostic) ->
                Printf.eprintf "  %s\n" (Lpf.Policy.diagnostic_to_string d))
              diagnostics;
  ...
  let require_policy_file message =
    ...
    | None ->
        ...
        exit 64
  in
  let port_or_zero = function Some p -> p | None -> 0 in
  let all_rules_supported _rule = true in

  match cmd with
  | "consistency" ->
      let file = require_policy_file "no policy file" in
      ...
      if dead_rules = [] then (
        ...
        exit 1)
  | "equivalence" -> (
      let file1 = require_policy_file "no first policy" in
      let file2 =
      ...
      | Lpf_z3.Z3_verify.Not_equivalent ne ->
          ...
          Printf.printf
            ...
            (port_or_zero ne.counterexample.port)
  ...
  | "reachable" -> (
      ...
      let target_action =
        ...
        | "pass" -> Lpf.Policy.Pass
        | "block" -> Lpf.Policy.Block
        | "reject" -> Lpf.Policy.Reject
        | _ ->
      ...
      | Lpf_z3.Z3_verify.Reachable ce ->
          Printf.printf
            "REACHABLE: packet exists that matches constraints and produces %s\n"
            action_str;
          Printf.printf "  src=%s dst=%s port=%d\n" ce.source ce.destination
            (port_or_zero ce.port);
          exit 0
      | Lpf_z3.Z3_verify.Unreachable ->
          Printf.printf
            "PASS: no packet matches constraints and produces %s (unreachable)\n"
            action_str;
          exit 0)
  | "invariant" -> (
      let file = require_policy_file "no policy file" in
      ...
      if invs = [] then (
        Printf.eprintf
          "no invariants specified (use -i src=10.0.0.0/8 -i dst=0.0.0.0/0)\n";
        exit 64);
      ...
      | Lpf_z3.Z3_verify.Violated ce ->
          ...
          Printf.printf "  counterexample: src=%s dst=%s port=%d\n" ce.source
            ce.destination (port_or_zero ce.port);
          exit 1)
  | "minimize" ->
      let file = require_policy_file "no policy file" in
      let ir = parse_ir (read_policy file) in
  ...
  | "backend-ebpf" -> (
      ...
      match
        Lpf_z3.Z3_verify.check_backend_equivalence ~ir
          ~ebpf_rules:all_rules_supported
      with
      | Lpf_z3.Z3_verify.Equivalent ->
      ...
      | Lpf_z3.Z3_verify.Not_equivalent ne ->
          ...
          Printf.printf
            "  counterexample: src=%s dst=%s decision1=%s decision2=%s\n"
            ne.counterexample.source ne.counterexample.destination
            ne.decision_in_first ne.decision_in_second;
          exit 1)
  | "check-all" ->
      ...
      if dead = [] then Printf.printf "  PASS\n"
        ...
        List.iter
          (fun (d : Lpf_z3.Z3_verify.dead_rule) ->
            Printf.printf "    rule at line %d: %s\n" d.Lpf_z3.Z3_verify.line
              d.Lpf_z3.Z3_verify.action)
          dead);
      ...
      let unreachable =
        List.filter
          (fun (rc : Lpf_z3.Z3_verify.rule_coverage) -> not rc.reachable)
          coverage
      in
      ...
      Printf.printf "\n[3/4] minimize...\n%!";
      let _minimal, removed = Lpf_z3.Z3_verify.minimize ir in
