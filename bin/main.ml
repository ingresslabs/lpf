...
let handle_tools args =
  ...
  | Ok format ->
      if String.equal format "system-prompt" then (
        print_endline (Lpf.Json_util.string Tool_schema.system_prompt);
        exit 0)
      else
        let tools =
          List.filter
        ...
        let render =
          match format with
          | "jsonschema" -> Tool_schema.jsonschema
          | _ -> Tool_schema.openai
        in
...
let () =
  ...
  | _ :: "verify" :: args -> Verify_delegate.handle args
