let () =
  ...
  match cmd with
  | Some "VERSION" | None ->
    Lpf.Cni.handle_version ()
  | Some cmd_str ->
    let config_text = read_stdin () in
    if config_text = "" then begin
      Printf.eprintf "{\"cniVersion\":\"1.0.0\",\"code\":4,\"msg\":\"empty network config\",\"details\":\"\"}\n%!";
      exit 3
    end;
    ...
    | Ok command ->
      ...
      | Ok cfg ->
        ...
        let cid = match container_id with Some c -> c | None -> "unknown" in
        ...
        | Lpf.Cni.Add ->
          begin match Lpf.Cni.handle_add cfg netns_path cid ifn with
          | Error e ->
        ...
        | Lpf.Cni.Del ->
          begin match Lpf.Cni.handle_del cfg netns_path cid ifn with
          | Error e ->
            Printf.eprintf "%s\n%!" (Lpf.Cni.error_result 7 e);
            exit 7
          | Ok () -> ()
        ...
        | Lpf.Cni.Check ->
          begin match Lpf.Cni.handle_check cfg netns_path cid ifn with
          | Error e ->
